...
/*
 * //The Class BinarySearch and its main() method are exactly the same //as in
 * Iterative Search code, except that it calls //performBinarySearchRecursive()
 * method here public static boolean performBinarySearchRecursive(List<Integer>
 * integerList, Integer noToSearch, int low, int high) { if (low > high) {
 * return false; } else { int mid = (low + high) / 2; if
 * (integerList.get(mid).equals(noToSearch)) { System.out.println(noToSearch +
 * " found at index " + mid); return true; } else if (noToSearch <
 * integerList.get(mid)) { return performBinarySearchRecursive(integerList,
 * noToSearch, low, mid - 1); } else { return
 * performBinarySearchRecursive(integerList, noToSearch, mid + 1, high ); } } }
 */